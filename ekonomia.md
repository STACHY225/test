...
Docelowy zarobek **sprzedaży ulicznej**: **800–1500$/h** typowo | **1500–2200$/h** aktywna sesja — **górna połowa** widełek crime (miks 700–1200$/h); wyższe ryzyko policji, setup i rotacja sprzedaży.

**Produkcja** (pole / lab / KQ) ma być **opłacalniejsza niż legalne side joby** (500–850$/h), ale **wolniejsza** niż sam spam dealów z gotowego towaru — gracz płaci czasem na zbieranie, craft i logistykę.

| Tryb | Docelowe $/h (brutto, przed praniem) | vs legal | Uwagi |
|---|---:|---|---|
| Side job (kotwica) | 500–850 | — | miner, KQ, itd. |
| Produkcja weed (pole + KQ) | **700–1100** | +40–60% | wolniejszy cykl niż sam deal |
| Produkcja twarda (koka / meta) | **900–1400** | +80–100% | setup, składniki, ryzyko |
| Sprzedaż uliczna (gotowy towar) | 800–2200 | — | cap E3; nie farmić bez CD |

Szczegóły łańcuchów produkcji — sekcje **rage_drugs**, **rage_meth**, **KQ weed** poniżej.
...
## Produkcja narkotyków — zasady balansu

| Zasada | Wartość |
|---|---|
| Cel względem legalu | **Wyższe $/h niż side job**, bo crime + ryzyko PD |
| Cel względem dealów | **Niższe lub równe** spamowi ulicy — produkcja = setup, nie AFK print |
| Tempo | Wydłużone **czasy harvest / respawn / craft**; brak burst mnożników ×3–×4 |
| Test QA | Pełny łańcuch: zbierz → przetwórz → spakuj → sprzedaj (patrz `ekonomia-qa-testy.md` E6, E10–E12) |

### rage_drugs — pole i stoły (`[rage]/rage_drugs/Config.lua`)

Łańcuchy: **weed_plant → weed → weed_packed** (5 weed + woreczek) · **coke_leaf → coke_packed** · **heroin_poppy → opium → heroin_packed** · **magic_mushrooms → dry**.

| Strefa / etap | Parametr | Obecnie | Docelowo v2 |
|---|---|---|---|
| Weed harvest | `duration` | 8500 ms | **10 000–11 000 ms** |
| Weed harvest | `spawnDelay` | 25–30 s | **35–40 s** |
| Weed harvest | `multiplier` | 1–2 / 1–3 | **1–2** (oba pola) |
| Weed process | `multiplier` | 1–4 (×2 weed) | **1–2** (×2 weed) |
| Weed process | `duration` | 8500 ms | **10 000 ms** |
| Koka harvest | `duration` | 11 500 ms | **12 000–13 000 ms** |
| Koka harvest | `spawnDelay` | 30 s | **40–45 s** |
| Koka process | `multiplier` | 1–3 (×2 packed) | **1–2** |
| Koka process | `duration` | 13 000 ms | **14 000–15 000 ms** |
| Grzyby harvest | `spawnDelay` | 15 s | **25–30 s** |
| Pakowanie weed | 5× weed → 1× packed | bez zmian | bez zmian |
| `empty_baggy` | koka / heroina | wymagany | bez zmian |

**Szacunek $/h (solo, v2 ceny ulicy, 2 serie testowe QA):**

| Łańcuch | Czas ~ (2 serie) | Przychód netto ~ | $/h docelowo |
|---|---:|---:|---:|
| Weed pole → packed → ulica | 45–70 min | 550–900$ | **700–1000** |
| Koka pole → packed → ulica | 50–75 min | 650–1100$ | **850–1200** |
| Grzyby → dry → ulica | 35–55 min | 350–600$ | **500–750** (entry crime) |

**Anty-farm:** przy obecnych mnożnikach ×3–×4 i `spawnDelay` 15–30 s pole weed daje **2–4×** docelowe $/h — wymaga rescale przed deployem.

### rage_meth — lab mobilny (`[rage]/rage_meth/`)

Gotowanie w pojeździe (`journey` + `methlab`). Składniki: aceton (pole `rage_drugs`), lit, woreczek.

| Parametr | Obecnie | Docelowo v2 |
|---|---|---|
| `NeededItems.acetone` | 5 | **6–8** |
| Yield `meth_packed` / cook | `random(5, 15)` | **`random(3, 8)`** |
| `Config.accetonTime` (+ mask, nausea) | 20 000 ms | **25 000–30 000 ms** |
| Czas aktywny cook (UI + pytania) | ~3–8 min | **10–18 min** |
| Cooldown / smoke PD | istnieje | bez zmian (ryzyko) |

| Metryka | Docelowo |
|---|---|
| $ za udany cook (sprzedaż v2) | **400–900$** brutto |
| $/h pełny cykl (zbiór + cook + 6–8 dealów) | **900–1400** |
| Min. czas cyklu | **≥25 min** (nie szybciej) |

Szczegóły KQ weed (odmiany, czasy grow) — sekcja **KQ weed — produkcja** dalej w dokumencie.
...
**Cel balansu:** produkcja KQ weed **opłacalniejsza niż legalne side joby** (500–850$/h), ale **nie szybsza** niż typowy crime i **nie przewyższa** spamu dealów ulicznych. Solo farm: **700–1100$/h**; wyższe odmiany wymagają warunków (grow tent / lockup) i dłuższego cyklu. Ryzyko: policja, konfiskata, koszt setupu (nasiona, nawóz, woda).
...
| Narkotyki, produkcja (rage_drugs, rage_meth, KQ weed), lombard, pralnia | Opisane |
...
- rage_drugs / rage_meth — produkcja: tempo harvest+craft, $/h > legal, < spam dealów (§ Produkcja narkotyków).
